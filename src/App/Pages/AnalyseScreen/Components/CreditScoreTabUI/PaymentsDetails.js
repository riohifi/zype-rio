...
const PaymentsDetails = ({ handleUISelect, Styles , catList, setCatLis}) => {
    ...
    return (
        <>
            <View style={{ paddingBottom: wp('20%'), flex: 1, alignItems: 'center' }}>
                ...
                <View style={[Styles.row_1, {marginBottom: wp('5%')}]}>
                    {/* *********** Row 1 Start ******* */}
...

                {
                    bankList.map((item, i)=>(
                        ...
                        <TouchableOpacity onPress={() => { handleNextGo(item) }} style={[Styles.row_1_row,]}>
                            ...
                            <View>
                                {/* <Text style={[Styles.row_1_row_text, { fontSize: wp('3%'), color: Colors.black }]}>{item.date}</Text> */}
                            </View>
                        </TouchableOpacity>
                        {/* *********** Row 3 Start ******* */}
                        <View style={[Styles.row_1_row, { marginTop: wp('4%') }]}>
                            <View>
                                <Text style={[Styles.row_1_row_btn_text_2, {color: Colors.black, fontFamily: Fonts.regular}]}>Interest Rate: <Text style={{fontFamily: Fonts.bold, fontSize: item.rate_of_interest ? wp('5%') : wp('3%'),}}>{item.rate_of_interest ? `${item.rate_of_interest}%` : ' --'}</Text></Text>
                            </View>
                            <TouchableOpacity  onPress={() => { handleNextGo(item) }} style={{ flexDirection: 'row', alignItems: 'center' }}>
                                <Text style={[Styles.row_1_row_btn_text_2, {color: Colors.black, fontFamily: Fonts.regular}]}>View history</Text>
...
export default observer(PaymentsDetails)